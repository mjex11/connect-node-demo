...
import { createAsyncIterable, createMethodSerializationLookup, createMethodUrl, pipe, pipeTo, sinkAllBytes, transformCompressEnvelope, transformDecompressEnvelope, transformJoinEnvelopes, transformNormalizeMessage, transformParseEnvelope, transformSerializeEnvelope, transformSplitEnvelope, } from "@bufbuild/connect/protocol";
import { appendHeaders, Code, ConnectError, runStreaming, runUnary, } from "@bufbuild/connect";
import { createEndStreamSerialization, requestHeaderWithCompression, endStreamFlag, errorFromJsonBytes, headerUnaryContentLength, headerUnaryEncoding, trailerDemux, validateResponseWithCompression, } from "@bufbuild/connect/protocol-connect";
...
export function createConnectTransport(options) {
    ...
    return {
        async unary(service, method, signal, timeoutMs, header, message) {
            const serialization = createMethodSerializationLookup(method, options.binaryOptions, options.jsonOptions, opt);
            return await runUnary({
                ...
                const { compression, isUnaryError, unaryError } = validateResponseWithCompression(method.kind, opt.useBinaryFormat, opt.acceptCompression, universalResponse.status, universalResponse.header);
        ...
        async stream(service, method, signal, timeoutMs, header, input) {
            const serialization = createMethodSerializationLookup(method, options.binaryOptions, options.jsonOptions, opt);
            const endStreamSerialization = createEndStreamSerialization(options.jsonOptions);
            return runStreaming({
                stream: true,
                service,
                method,
            ...
            async (req) => {
                ...
                const { compression } = validateResponseWithCompression(method.kind, opt.useBinaryFormat, opt.acceptCompression, uRes.status, uRes.header);
